...
foreach ($fotos as $key => $value) {
  ...
  <div class="card-body">
    <h4 id="A<?php echo s($value->id) ?>" class="<?php 
    if($value->aprobado === "N"){
        echo "btn btn-danger";     
    }else{
 ...
 if ($value->aprobado === "N") {
  ?>
    ...
    <input id="B<?php echo s($value->id) ?>" data-id="<?php echo s($value->id) ?>" type="submit"class="aprobar btn btn-primary mt-2" value="Approve">
    </form>
  <?php
 }
 ?>
...
<button id="btntcarga" type="button" class="btn btn-primary p-4 loginS mt-3">Load more</button>
 ...
 $script .= '<script src="/build/js/cargaadmin.js"></script>';
